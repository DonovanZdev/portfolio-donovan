...
- **Hosting:** **Cloudflare Pages** (proyecto `portfolio-donovan`, URL `portfolio-donovan.pages.dev`). El sitio es un **export estático** (`output: "export"`): `next build` genera `./out` y `wrangler pages deploy` lo sube (`wrangler.jsonc` con `pages_build_output_dir`). Los headers de seguridad y el `Content-Type` de las imágenes OG viven en `public/_headers`.
- **Por qué Pages y no Workers:** una URL de Worker es siempre `<worker>.<subdominio-de-la-cuenta>.workers.dev` y el subdominio de esta cuenta es `donovanadrianpro` (no se puede cambiar a `donovan`: ya está tomado). Pages da `portfolio-donovan.pages.dev`, más corta. Para un sitio 100% estático no se pierde nada.
...
- **Scripts:** `pnpm dev` (desarrollo), `pnpm build` (genera `./out`), `pnpm preview` (build + `wrangler pages dev`), `pnpm run deploy` (build + `wrangler pages deploy`; **con `run`**, `pnpm deploy` es otro comando).
...
Fase: **en línea**. El sitio ya tiene el contenido de Donovan (hero con foto, sobre mí con cifras, stack, 5 casos, contacto por WhatsApp y correo) y un blog con 5 artículos en español, uno por caso. Compila como export estático (18 páginas), verificado con `wrangler dev` en escritorio y celular. **Desplegado el 2026-09-18 en https://portfolio-donovan.pages.dev.** Se quitaron las secciones Work, Education y Hackathons porque el PDF no trae esos datos: agregarlas cuando Donovan los dé (había un `WorkSection`/`HackathonsSection` en el template; recuperables con `git show 6500c12:src/components/section/work-section.tsx`).
