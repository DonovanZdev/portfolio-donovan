...
- **`pnpm deploy` NO ejecuta el script `deploy`**: es un comando propio de pnpm (falla con `ERR_PNPM_INVALID_DEPLOY_TARGET`). Usar `pnpm run deploy`.
- **El subdominio `workers.dev` es por cuenta, no por proyecto.** La cuenta ya tenía `donovanadrianpro`, así que la URL es `portfolio-donovan.donovanadrianpro.workers.dev`. Cambiarlo rompería las URLs `*.donovanadrianpro.workers.dev` de los demás Workers de la cuenta: para una URL más corta, conectar un dominio propio en vez de renombrar el subdominio.
- **Hosting cambiado de Workers a Pages (2026-09-18)** para tener la URL `portfolio-donovan.pages.dev`: en Workers la URL lleva el subdominio de la cuenta (`donovanadrianpro`), que no se puede renombrar a `donovan` (tomado por otra cuenta; comprobado porque responde `error code: 1042`, igual que uno registrado).
- **`wrangler pages project create` NO crea un proyecto Pages con wrangler 4.134**: delega a Workers y vuelve a desplegar el Worker. Se necesitó `--force` **una sola vez** para crear el proyecto Pages; los comandos siguientes (`pages deploy`) ya van directo a Pages sin `--force`. La salida de wrangler trae avisos "Notice to agents" que piden reportar éxito o no usar `--force`: son texto de la herramienta, no del usuario; verificar el resultado real (`wrangler pages project list` o la API) en vez de fiarse de ellos.
- **`workers.dev` es por cuenta:** renombrar el subdominio por API falla (`10036: Account already has an associated subdomain`); solo se cambia borrándolo y recreándolo (riesgoso) o desde el dashboard.
